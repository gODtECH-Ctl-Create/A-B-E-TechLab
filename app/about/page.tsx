...
export default function About() {
  return (
    <div>
      ...
      <section className="container py-24 md:py-32">
        <div className="grid gap-12 md:grid-cols-[.75fr_1.25fr]">
          ...
          <div className="max-w-2xl text-lg leading-8 text-black/65">
            <p>Great ideas often get stuck between seeing an opportunity and building the right thing. ABE TechLab exists to close that gap.</p>
            <p className="mt-7">We work across research, product strategy, product marketing, design and technology. Sometimes that means building our own products. Sometimes it means working closely with an organisation to shape and develop a digital system around a real problem.</p>
            <p className="mt-7">Education is another part of the studio. Through TechTrack and future technology programmes, we create practical learning, bootcamp and internship experiences that connect technology with real work.</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-black/10 bg-[#ecece6]">
        <div className="container py-24 md:py-32">
          <div className="mb-12 max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-black/40">What we do</p>
            <h2 className="font-display mt-5 text-4xl font-semibold leading-none tracking-[-.055em] md:text-6xl">From understanding the problem to building what comes next.</h2>
          </div>
          <div className="grid gap-px overflow-hidden border border-black/10 bg-black/10 md:grid-cols-2">
            {whatWeDo.map(([number, title, text]) => (
              <div key={number} className="bg-[#f5f5ef] p-8 transition hover:bg-white md:p-10">
      ...
      <section className="container py-24 md:py-32">
        <div className="mb-12">
          <p className="text-xs font-semibold uppercase tracking-[.16em] text-black/40">Founder</p>
          <h2 className="font-display mt-5 text-4xl font-semibold tracking-[-.055em] md:text-6xl">The person behind the studio.</h2>
        </div>
        <FounderCard />
      </section>
      ...
      <section className="border-y border-black/10 bg-[#f5f5ef]">
        <div className="container py-24 md:py-32">
          <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
            <div><p className="text-xs font-semibold uppercase tracking-[.16em] text-black/40">Team</p><h2 className="font-display mt-5 text-4xl font-semibold tracking-[-.055em] md:text-6xl">A growing team around the work.</h2><p className="mt-5 max-w-2xl text-base leading-7 text-black/55">ABE TechLab combines human product leadership with AI-assisted research and technology. More specialists can join as the studio grows.</p></div>
            <Link href="/team" className="inline-flex w-fit items-center gap-2 border border-black/15 px-5 py-3 text-sm font-semibold transition hover:border-black/40">Meet the team <ArrowUpRight size={16} /></Link>
          </div>
          <div className="mt-12 grid gap-px overflow-hidden border border-black/10 bg-black/10 md:grid-cols-2">
            {team.map((member) => (
              <article key={member.name} className="bg-white p-8 md:p-10">
                <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-black/40">{member.type}</p>
                <h3 className="font-display mt-6 text-3xl font-semibold tracking-[-.045em]">{member.name}</h3>
                <p className="mt-2 text-sm font-medium text-black/55">{member.role}</p>
                <p className="mt-5 max-w-xl text-sm leading-7 text-black/55">{member.bio}</p>
              </article>
            ))}
          </div>
        </div>
