...
export default function Home() {
  return (
    <>
      ...
      <section className="container py-28 md:py-36"><div className="grid gap-12 md:grid-cols-[.7fr_1.3fr]"><div><SectionLabel>What we do</SectionLabel><h2 className="font-display max-w-md text-4xl font-semibold leading-[.95] tracking-[-.055em] md:text-6xl">From a rough idea to something people can use and grow.</h2></div><div className="grid gap-4 sm:grid-cols-2">{services.map((service) => <div key={service.number} className="service-card group border border-black/10 bg-white p-7 transition duration-300 hover:-translate-y-1 hover:border-black/25 hover:shadow-[0_18px_45px_rgba(0,0,0,.07)]"><div className="flex items-center justify-between"><span className="text-[10px] font-semibold tracking-[.18em] text-black/35">{service.number}</span><ArrowUpRight size={18} className="text-black/25 transition group-hover:text-black" /></div><h3 className="font-display mt-14 text-2xl font-semibold tracking-[-.04em]">{service.title}</h3><p className="mt-3 text-sm leading-6 text-black/55">{service.text}</p><div className="mt-7 space-y-2 border-t border-black/10 pt-5">{service.items.slice(0, 3).map((item) => <p key={item} className="flex items-center gap-2 text-xs text-black/45"><Check size={13} />{item}</p>)}</div></div>)}</div></div></section>
      <section className="border-y border-black/10 bg-[#ecece6]"><div className="container py-24 md:py-32"><div className="mb-12 grid gap-8 md:grid-cols-[.7fr_1.3fr] md:items-end"><div><SectionLabel>Areas we build in</SectionLabel><h2 className="font-display text-4xl font-semibold leading-none tracking-[-.055em] md:text-6xl">Technology with a purpose.</h2></div><p className="max-w-xl text-sm leading-7 text-black/55">Our work is not limited to one industry. We build around real problems, including the way people learn, teach, build, work and access services.</p></div><div className="grid gap-px overflow-hidden border border-black/10 bg-black/10 md:grid-cols-3">{focusAreas.map((area, index) => <article key={area.title} className="bg-[#f5f5ef] p-8 md:p-10"><span className="text-[10px] font-semibold tracking-[.18em] text-black/30">0{index + 1}</span><h3 className="font-display mt-16 text-2xl font-semibold tracking-[-.04em]">{area.title}</h3><p className="mt-3 text-sm leading-7 text-black/55">{area.text}</p></article>)}</div></div></section>
      <section className="bg-[#11110f] py-28 text-white md:py-36"><div className="container"><div className="mb-14 grid gap-8 md:grid-cols-[.7fr_1.3fr] md:items-end"><div><SectionLabel>Selected work</SectionLabel><h2 className="font-display text-4xl font-semibold leading-none tracking-[-.055em] md:text-6xl">Products we've helped shape.</h2></div><p className="max-w-xl text-sm leading-7 text-white/50">A selection of digital products, platforms and systems across teaching and learning, construction, professional identity and recruitment technology.</p></div><div className="grid gap-5 md:grid-cols-2">{projects.slice(0, 4).map((project, index) => <ProjectVisual key={project.name} name={project.name} category={project.category} index={index} />)}</div><div className="mt-10"><Link href="/work" className="inline-flex items-center gap-2 border border-white/20 bg-transparent px-5 py-3 text-sm font-semibold text-white transition hover:border-white/50">See all work <ArrowUpRight size={16} /></Link></div></div></section>
      <section className="container py-28 md:py-36"><div className="grid gap-14 md:grid-cols-[.7fr_1.3fr]"><div><SectionLabel>How we work</SectionLabel><h2 className="font-display text-4xl font-semibold leading-[.95] tracking-[-.055em] md:text-6xl">Good products don't happen by accident.</h2><p className="mt-6 max-w-md text-sm leading-7 text-black/55">We give ideas structure, test assumptions, shape the market story and move them toward something people can actually use.</p></div><div className="process-list">{[["01","Research","Understand the problem, users, market and opportunity."],["02","Define","Turn evidence, requirements and ideas into a clear product direction."],["03","Design","Shape the experience, interface and product structure."],["04","Build","Develop, test and refine the product."],["05","Position & Launch","Help the product reach the right audience, learn and improve."]].map(([number,title,text]) => <div key={number} className="process-item group grid grid-cols-[52px_1fr_auto] gap-5 border-t border-black/15 py-7"><span className="text-xs text-black/30">{number}</span><div><h3 className="font-display text-2xl font-semibold tracking-[-.035em]">{title}</h3><p className="mt-2 max-w-xl text-sm leading-6 text-black/50">{text}</p></div><ArrowUpRight size={19} className="mt-1 text-black/20 transition group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-black" /></div>)}</div></div></section>
      <section className="border-y border-black/10 bg-[#ecece6]"><div className="container grid gap-12 py-28 md:grid-cols-[1fr_1fr] md:items-center md:py-36"><div><SectionLabel>Who we work with</SectionLabel><h2 className="font-display text-5xl font-semibold leading-[.92] tracking-[-.06em] md:text-7xl">Built for people building something.</h2></div><div className="grid gap-px overflow-hidden border border-black/10 bg-black/10 sm:grid-cols-2">{audiences.map(([title, text]) => <div key={title} className="bg-[#f5f5ef] p-7 transition hover:bg-white"><h3 className="font-display text-xl font-semibold tracking-[-.03em]">{title}</h3><p className="mt-3 text-sm leading-6 text-black/55">{text}</p></div>)}</div></div></section>
      <section className="border-b border-black/10 bg-[#ecece6]"><div className="container grid gap-12 pb-28 md:grid-cols-[1fr_1fr] md:items-center md:pb-36"><div><SectionLabel>About ABE TechLab</SectionLabel><h2 className="font-display text-5xl font-semibold leading-[.92] tracking-[-.06em] md:text-7xl">Small studio.<br />Serious products.</h2></div><div className="max-w-xl text-base leading-8 text-black/60"><p>ABE TechLab works at the intersection of research, product strategy, design, technology and product marketing. We help turn real problems into digital products and systems, from early discovery through development and launch.</p><p className="mt-5">Education is a core part of that work. Through TechTrack, we build teaching and learning technology that supports structured learning, practical projects, bootcamps and professional development.</p><p className="mt-5">We don't believe every project needs a large agency. It needs the right thinking, the right people and a clear path to shipping, positioning and learning from the market.</p></div></div></section>
      <section className="cta-section relative overflow-hidden bg-[#b7ff3c]"><div className="absolute -right-20 top-1/2 h-80 w-80 -translate-y-1/2 rounded-full bg-white/40 blur-[100px]" /><div className="container relative flex flex-col gap-10 py-28 md:flex-row md:items-end md:justify-between md:py-36"><div><SectionLabel>Start a conversation</SectionLabel><h2 className="font-display max-w-3xl text-5xl font-semibold leading-[.9] tracking-[-.065em] md:text-8xl">Have something worth building?</h2><p className="mt-7 max-w-xl text-base text-black/60">Tell us what you're working on. We'll help you figure out what comes next.</p></div><Link href="/contact" className="btn-primary shrink-0 px-7 py-5 text-sm font-semibold transition">Start a project <ArrowUpRight size={18} /></Link></div></section>
    </>
  );
}
