...
function FooterLinks({ links }: { links: readonly (readonly [string,string])[] }) {
  return <ul className="mt-4 space-y-3 text-sm text-black/60">{links.map(([label, href]) => <li key={href}><Link href={href} className="transition hover:text-black">{label}</Link></li>)}</ul>;
}
  ...
  return <footer className="border-t border-black/10 bg-[#f5f5f2]">
    <div className="container py-14 md:py-16">
      <div className="grid gap-12 md:grid-cols-[1.5fr_repeat(4,1fr)] md:gap-8 lg:gap-12">
        ...
        <div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-black/40">Connect</p><FooterLinks links={connectLinks}/><a href="mailto:abeayo6@gmail.com" className="mt-3 inline-block text-sm text-black/60 transition hover:text-black">Email</a><span className="mt-2 block text-xs text-black/35">Socials coming soon</span></div>
