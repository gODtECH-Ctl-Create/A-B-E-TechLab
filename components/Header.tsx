...
export function Header(){
  ...
  return <header className="sticky top-0 z-50 border-b border-black/10 bg-[#f5f5f2]/95 backdrop-blur-md">
    <div className="container flex h-[68px] items-center justify-between gap-3 sm:h-[72px]">
      <Logo/>
      <nav aria-label="Primary navigation" className="hidden items-center gap-6 lg:flex">{links.map(([label,href])=><Link key={href} href={href} className="text-sm text-black/65 transition hover:text-black focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-black">{label}</Link>)}</nav>
      <Link href="/contact" className="btn-primary hidden px-5 py-3 text-sm font-semibold transition focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c] lg:inline-flex">Start a project</Link>
      <button
        type="button"
    ...
    {open&&<div id="mobile-navigation" className="border-t border-black/10 bg-[#f5f5f2] lg:hidden">
      <nav aria-label="Mobile navigation" className="container flex max-h-[calc(100svh-68px)] flex-col overflow-y-auto py-4 sm:max-h-[calc(100svh-72px)] sm:py-5">
        {links.map(([label,href])=><Link onClick={()=>setOpen(false)} key={href} href={href} className="min-h-12 border-b border-black/10 py-3 text-lg font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black">{label}</Link>)}
        <Link onClick={()=>setOpen(false)} href="/contact" className="btn-primary mt-5 min-h-12 px-5 py-4 text-center font-semibold focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c]">Start a project <span aria-hidden="true">↗</span></Link>
      </nav>
    </div>}
  </header>
}
