...
export default function MobileContactForm() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!need || !timeline || !details.name.trim() || !details.email.trim() || !details.message.trim()) return;
    setSending(true);
    setStatus('idle');
  ...
  return (
    <form onSubmit={submit} className="mobile-contact-form" noValidate>
      <div className="mb-6 flex items-center justify-between text-[10px] font-semibold uppercase tracking-[.18em] text-black/40" aria-label="Contact form progress">
        ...
        {step > 1 && step < 4 && <button type="button" onClick={() => setStep(step - 1)} className="inline-flex min-h-11 items-center gap-2 text-black/60 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"><ArrowLeft size={15}/> Back</button>}
      </div>
      ...
      {step === 1 && (
        <div>
          ...
          <div className="mt-7 grid gap-2">
            {needs.map((item) => (
              <button key={item} type="button" onClick={() => { setNeed(item); setStep(2); }} className={`mobile-choice focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black ${need === item ? 'is-selected' : ''}`}>
      ...
      {step === 2 && (
        <div>
          ...
          <div className="mt-7 grid gap-5">
            <label className="block"><span className="mobile-field-label">Name</span><input required name="name" value={details.name} onChange={(e) => setDetails((current) => ({ ...current, name: e.target.value }))} autoComplete="name" className="mobile-field focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black" /></label>
            <label className="block"><span className="mobile-field-label">Email</span><input required name="email" value={details.email} onChange={(e) => setDetails((current) => ({ ...current, email: e.target.value }))} type="email" inputMode="email" autoComplete="email" className="mobile-field focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black" /></label>
            ...
            <label className="block"><span className="mobile-field-label">Project description</span><textarea required name="message" value={details.message} onChange={(e) => setDetails((current) => ({ ...current, message: e.target.value }))} rows={5} placeholder="What are you trying to build, research or improve?" className="mobile-field resize-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black" /></label>
          </div>
          {detailsError && <p className="mt-4 text-sm leading-6 text-black/55" role="alert">{detailsError}</p>}
          <button type="button" onClick={continueToTiming} className="mt-7 btn-primary w-full px-5 py-4 text-sm font-semibold focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c]">Continue <ArrowUpRight size={16}/></button>
        </div>
      )}
      ...
      {step === 3 && (
        <div>
          ...
          <div className="mt-7 grid gap-2">
            {timelines.map((item) => <button key={item} type="button" onClick={() => setTimeline(item)} aria-pressed={timeline === item} className={`mobile-choice focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black ${timeline === item ? 'is-selected' : ''}`}><span>{item}</span>{timeline === item ? <Check size={17} aria-hidden="true"/> : <span aria-hidden="true" className="text-black/30">→</span>}</button>)}
          </div>
          <button type="submit" disabled={!timeline || sending} className="mt-7 btn-primary w-full px-5 py-4 text-sm font-semibold focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c] disabled:cursor-not-allowed disabled:opacity-50">{sending ? 'Sending…' : 'Send enquiry'} <ArrowUpRight size={16}/></button>
