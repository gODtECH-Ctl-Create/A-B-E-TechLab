...
const followUpChannels = [
  { value: 'website_chat', label: 'Chat with our assistant here', description: 'Continue on the website.' },
  { value: 'whatsapp', label: 'Continue on WhatsApp', description: 'Take the conversation to WhatsApp.' },
  { value: 'voice_call', label: 'Have our assistant call me', description: 'Continue by phone.' },
  { value: 'email', label: 'Continue by email', description: 'Keep the conversation in your inbox.' },
];
...
export default function MobileContactForm() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (sending || !need || !timeline || !preferredChannel || !details.name.trim() || !details.email.trim() || !details.message.trim()) return;
    setSending(true);
    setStatus('idle');
    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ ...details, need, timeline, preferred_channel: preferredChannel }),
      });
  ...
  return (
    <form onSubmit={submit} className="mobile-contact-form" noValidate aria-busy={sending}>
      <div className="mb-6 flex items-center justify-between text-[10px] font-semibold uppercase tracking-[.18em] text-black/40" aria-label="Contact form progress">
        <span aria-live="polite">{step < 5 ? `Step ${step} of 4` : 'Sent'}</span>
        {step > 1 && step < 5 && (
      ...
      {step === 3 && (
        <div>
          ...
          <button type="button" onClick={continueToChannel} disabled={!timeline} className="mt-7 btn-primary inline-flex min-h-14 w-full items-center justify-center gap-2 px-5 py-4 text-sm font-semibold focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c] disabled:cursor-not-allowed disabled:opacity-50">Continue <ArrowUpRight size={16} aria-hidden="true"/></button>
        </div>
      )}

      {step === 4 && (
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[.18em] text-black/40">Your choice</p>
          <h2 className="font-display mt-3 text-3xl font-semibold leading-tight tracking-[-.045em]">How would you like to continue?</h2>
          <p className="mt-3 text-sm leading-6 text-black/55">Choose where you'd like our assistant to continue the conversation after you submit.</p>
          <div className="mt-7 grid gap-2">
            {followUpChannels.map((channel) => <button key={channel.value} type="button" disabled={sending} onClick={() => setPreferredChannel(channel.value)} aria-pressed={preferredChannel === channel.value} className={`mobile-choice items-start text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#b7ff3c] ${preferredChannel === channel.value ? 'is-selected' : ''}`}><span><span className="block">{channel.label}</span><span className="mt-1 block text-xs font-normal leading-5 text-black/45">{channel.description}</span></span>{preferredChannel === channel.value ? <Check size={17} aria-hidden="true"/> : <span aria-hidden="true" className="text-black/30">→</span>}</button>)}
          </div>
          <button type="submit" disabled={!preferredChannel || sending} aria-disabled={!preferredChannel || sending} className="mt-7 btn-primary inline-flex min-h-14 w-full items-center justify-center gap-2 px-5 py-4 text-sm font-semibold focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-[#b7ff3c] disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {step === 5 && (
        <div className="py-8 text-center" role="status" aria-live="polite">
          ...
          <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-black/55">Your enquiry has been sent. We'll continue the conversation through your selected channel.</p>
