...
function whatsappHref(leadId: string | null) {
  ...
  const text = `Hi ABE TechLab Assistant. I'd like to continue my enquiry.${leadId ? ` Lead: ${leadId}` : ""}`;
...
export default function WebsiteAssistant() {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("storage", onContext); window.removeEventListener("abe-assistant-context", onContext); };
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`/api/assistant/chat?lead_id=${encodeURIComponent(leadId)}`, { cache: "no-store" })
      ...
      .then((body) => {
        ...
        else return fetch("/api/assistant/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lead_id: leadId, channel: "website_chat" }) }).then((response) => response.json()).then((first) => { if (first?.message) setMessages([first.message]); });
      })
  ...
  async function send(event: FormEvent) {
    ...
    const local: ChatMessage = { id: `local-${Date.now()}`, sender_type: "client", content: text, created_at: new Date().toISOString() };
  ...
  return <>
    ...
    {open && <div className="website-assistant-shell" role="dialog" aria-label="ABE TechLab Assistant">
      ...
      {!leadId ? <div className="website-assistant-empty"><div className="website-assistant-mark">✦</div><strong>Let's start with your project enquiry.</strong><p>Complete the short enquiry form first. Once it is submitted, I can continue with you here and keep your conversation connected to our Operations workspace.</p><a href="/contact" onClick={() => setOpen(false)}>Start an enquiry →</a></div> : <>
        <div className="website-assistant-context"><span>Connected enquiry</span><strong>{leadId.slice(0, 8)}…</strong></div>
        <div className="website-assistant-messages" aria-live="polite">
          {messages.map((message) => <div key={message.id} className={`website-assistant-message ${message.sender_type === "client" ? "from-client" : "from-assistant"}`}><span>{message.sender_type === "client" ? "You" : "ABE Assistant"}</span><p>{message.content}</p></div>)}
          {busy && <div className="website-assistant-message from-assistant"><span>ABE Assistant</span><p className="assistant-typing">Thinking…</p></div>}
          <div ref={endRef}/>
        </div>
