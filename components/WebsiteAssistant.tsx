...
const CALL_NUMBER = process.env.NEXT_PUBLIC_VOICE_ASSISTANT_NUMBER || "";
...
function callHref() {
  ...
  return `tel:${CALL_NUMBER.replace(/\s/g, "")}`;
}
...
export default function WebsiteAssistant() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("abe-assistant-context", onContext);
      window.removeEventListener("abe-assistant-open", onOpen);
    };
  }, []);
    ...
    fetch(`/api/assistant/chat?lead_id=${encodeURIComponent(leadId)}`, { cache: "no-store" })
      ...
      .then(async (body) => {
        ...
        const response = await fetch("/api/assistant/chat", {
          ...
          body: JSON.stringify({ lead_id: leadId, channel: "website_chat" }),
        });
  ...
  async function send(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/assistant/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lead_id: leadId, channel: "website_chat", message: text }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Assistant unavailable");
  ...
  return <>
    <button type="button" className={`website-assistant-launcher ${open ? "is-open" : ""}`} onClick={() => setOpen(true)} aria-label="Open ABE TechLab assistant">
      <MessageCircle size={20}/><span>Assistant</span>
    </button>
    {open && <div className="website-assistant-shell" role="dialog" aria-label="ABE TechLab Assistant">
      ...
      {!leadId ? <div className="website-assistant-empty"><div className="website-assistant-mark">✦</div><strong>Let's start with your project enquiry.</strong><p>Complete the short enquiry form first. Once it is submitted, I can continue with you here and keep your conversation connected to Operations.</p><a href="/contact" onClick={() => setOpen(false)}>Start an enquiry →</a></div> : <>
        ...
        {call && <a className="website-assistant-call" href={call}>Call the AI assistant</a>}
